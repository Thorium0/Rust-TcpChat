...
fn display_info_msg(msg: &str, kind: &str) {
  ...
  msg_json.insert("kind", kind).expect("Could not insert kind on info-message");
...
fn send_msg(msg: String)  {
  ...
  msg_json.insert("user", USER.lock().unwrap().to_string()).expect("Json insert failed");
...
fn add_to_chatbox(window: Window) {
  thread::spawn(move || {
    ...
    loop {
      ...
      window.emit("add_to_chatbox", Message { message: msg, user }).unwrap();
      *MSG_TO_SEND.lock().unwrap() = String::new();
      ...
      window.emit("add_info_to_chatbox", Info { message: msg, kind }).unwrap();
